...
- `FGC`: 文字色
- `BGC`: 配置済みセルが保持する背景色
- `キャンバスBGC`: 未配置セルや `BGC:0` の背景表示に使う色
...
- `canvasBGC` は仕様書上の `キャンバスBGC`。
- `Invert BG` は `canvasBGC` の白/黒だけをトグルする。
...
- 透明または未配置部分だけ `キャンバスBGC` を反映して表示する。
