...
ターミナルまたは MDS のコマンドリンクからも起動できます。
...
```mds
<a href="cmd:matrix-rain">Open Matrix Rain</a>
```
...
- ストリーム生成時に、スロット `0` から `24` のうち 1 個以上 4 個以下を重複なしでランダムに選びます。
- 選ばれたスロットは、そのストリームが消えるまで固定します。
- 選ばれたスロットに文字が表示された後は、毎秒 6 回の固定頻度で現在と異なる文字へ差し替えます。
- 差し替え文字は通常の文字生成と同じ文字セットから選びます。
- ストリーム再生成時に、差し替え対象数と対象スロットを選び直します。

## 文字の寿命

- 初回の生成開始時刻は、全ストリームを 3 秒周期内へ等間隔に分散します。
- 再生成時は追加のランダム待機を入れず、各ストリームの生成タイミングの位相差を維持します。
- 各文字は個別の生成時刻を持ちます。
